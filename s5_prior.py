...
'''
Script to build prior for multi-reads allocation. The prior is built based on uniquely bin pair within valid interaction distance.
Feb, 2017
'''
...
def main():
	...
	# # - Ye
	# ######
	# # Write out the spline results
	# splineOut = open(outdir + "/splineResults" , "w")
	# if len(splineX) != len(splineY):
	# 	print("Error! spline x length does not equal to spline y length!")
	# 	sys.exit()
	# for iter in range(len(splineX)):
	# 	splineOut.write("%d" % splineX[iter] + '\t' + '{0:.10f}'.format(splineY[iter]) + '\n')
...
def fit_Spline(x,y,yerr,infilename,sortedInteractions,biasDic,figname,passNo):
	...
	tempMaxX=max(x)
	tempMinX=min(x)
	tempList=sorted(list(set([int(float(i[0])) for i in sortedInteractions])))
	splineX=[]
	...
	# Write out the spline results
	splineOut = open(outdir + "/splineResults" , "w")
	if len(splineX) != len(newSplineY):
		print("Error! spline x length does not equal to spline y length!")
		sys.exit()
	for iter in range(len(splineX)):
		if newSplineY[iter] > 0.0000000000000099999:
			splineOut.write("%d" % splineX[iter] + '\t' + '{0:.15f}'.format(newSplineY[iter]) + '\n')
	...
	for line in infile:
		...
		interxn=myUtils.Interaction([words[0], int(float(words[1])), words[2], int(float(words[3]))])
		interxn.setCount(int(float(words[4])))
		chr1=words[0]
		chr2=words[2]
	...
	infile =open(infilename, 'r')
	outfile =open(outdir+'/'+figname+'.significances.txt', 'w+')
	sys.stderr.write("Writing p-values to file %s" % figname + ".significances.txt\n")
	count=0
	...
	for line in infile:
		...
		chrNo2=words[2]
		midPoint2=int(words[3])
		interactionCount=int(float(words[4]))
		p_val=p_vals[count]
		q_val=q_vals[count]
...
def calculate_Probabilities(sortedInteractions,isOutlier,figname):
	...
	totalInteractionCountSum = 0
	lcount=0

	##############################################################
	#Ye - print out total number of contact with/without outliers
	##############################################################
	for eachrow in sortedInteractions:
		if isOutlier[lcount]==0:
			totalInteractionCountForPlot += eachrow[1]
		totalInteractionCountSum += eachrow[1]
		lcount+=1

	totalInteractionCountOut = open(outdir + "/totalInteractionCountResults" , "w")
	totalInteractionCountOut.write("%d" % totalInteractionCountSum + '\t' + "%d" % totalInteractionCountForPlot + '\n')
...
def generate_FragPairs(infilename):
	...
	################################################################
	# - Ye modified so that we only need fragment chr and midPoint #
	################################################################
